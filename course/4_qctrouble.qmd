...
### GENERATE TOY DATA ###
# NOTE: This is not important for understanding how to plot, but kept here
# in case you want to adjust some of these parameters to see what happens!
...
```{r}
#| echo: false
reactable::reactable(round(expression_data, digits = 2), defaultPageSize = 5, showPageSizeOptions = T)
```
...
```{r}
#| echo: false
reactable::reactable(metadata, defaultPageSize = 5, showPageSizeOptions = T)
```
...
```{r}
#| echo: false
reactable::reactable(expression_long, defaultPageSize = 5)
```
...
dist_matrix_samples <- dist(t(expression_data), method = "euclidean")
...
# Perform hierarchical clustering:
## 1. Compute a sample-wise distance matrix
dist_matrix_genes <- dist(expression_data, method = "euclidean")
## 2. Perform hierarchical clustering
...
metadata_plot <- ggplot(metadata, aes(x = fct_relevel(Sample, ordered_samples))) +
  ...
  scale_fill_nejm(guide = guide_legend(order = 1, 
                                 keywidth = guide_kwd, 
                                 keyheight = guide_kht)) +
  ggnewscale::new_scale_fill() +
  geom_tile(aes(y = "Drug", fill = Drug), color = "black") +
  scale_fill_aaas(guide = guide_legend(order = 2, 
                                 keywidth = guide_kwd, 
                                 keyheight = guide_kht)) +
  ggnewscale::new_scale_fill() +
  ...
  scale_y_discrete(limits = c("Condition", "Drug", "Gender"), 
                   expand = c(0,0)) +
...
genexp_boxplot <- genexp_boxplot + theme(axis.text.y = element_blank(),
...
Another great package for composing complex heatmaps is the aptly named [`ComplexHeatmap`](https://jokergoo.github.io/ComplexHeatmap-reference/book/). While some like the customization and syntax of `ggplot2`, the `ComplexHeatmap` can accomplish this figure with arguably far less effort. See the example below using the same code:
...
Heatmap(matrix = expression_data, 
        name = "Expression", 
...
meta_cols <- list(
  "Condition" = c("Case" = "#BC3C29FF", 
                  "Control" = "#0072B5FF"),
  "Drug" = c("DrugA" = "#3B4992FF",
             "DrugB" = "#EE0000FF",
             "DrugC" = "#008B45FF",
             "DrugD" = "#631879FF"),
  "Sex" = c("Male" = "#E64B35FF",
...
top_anno = HeatmapAnnotation(df = meta_ht,
                       col = meta_cols)
